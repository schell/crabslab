// Tests that don't depend on wire types or SPIR-V shaders.
...
// ---------------------------------------------------------------------------
// Wire-type-dependent and GPU shader tests — disabled until Phase 3.
//
// These tests depend on `craballoc-test-wire-types`, `crabslab::{Array, Id}`,
// and SPIR-V compute shaders, all of which are being migrated in Phase 3.
// ---------------------------------------------------------------------------

#[cfg(any())] // Gated: depends on Phase 3 wire types and SPIR-V shaders.
mod phase3_tests {
    //! This code is not compiled until Phase 3 updates the wire types and
    //! compute shaders to use crabslab2.
    ...
    // All tests from arena_roundtrip_sanity (Data portion),
    // array_subslice_sanity, gpu_update_test_*, proptest_gpu_updates_*,
    // workgroup_dimensions_to_id_sanity, invocations_sanity, regression —
